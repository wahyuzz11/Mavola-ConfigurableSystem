                    ...
                    <div class="d-flex">
                        @foreach ($shippingMethods as $index => $method)
                            <div class="form-check me-3">
                                <input type="radio" name="delivery_method" class="form-check-input delivery-method-radio"
                                    value="{{ $method->code }}" id="deliveryMethod{{ $method->id }}"
                                    data-code="{{ $method->code }}" {{ $index === 0 ? 'checked' : '' }}>
    ...
    <script>
        ...
        $('#salesOrderForm').on('submit', function(e) {
            ...
            const formData = {
                ...
                delivery_method: $('.delivery-method-radio:checked').val(),
                cashback: hasCashback && $('#enableCashback').is(':checked') ?
                    (parseFloat($('#cashbackInput').val()) || 0) : 0,
                global_discount: hasGlobalDiscount ? (parseFloat($('#globalDiscountAmount').val()) || 0) : 0
            };

            // Add delivery-specific fields if DEL-02 is selected
            if ($('.delivery-method-radio:checked').data('code') === 'DEL-02') {
                formData.delivery_cost = parseFloat($('#delivery_cost').val()) || 0;
                formData.recipient_name = $('#recipient_name').val();
                formData.customer_address = $('#customer_address').val();
            }
